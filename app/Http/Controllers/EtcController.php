...
class EtcController extends Controller
{
    ...
    public function products(Request $request) {
        ...
        return view('products', compact('request', 'orderedAuctions', 'orderedAuctionTypes','categories','categoriestype','pricecount','pricesort','priceg'));
    }
    ...
    public function win_lose(Request $request, Auction $auction){

            $endedAuctions = $auction->status('sold');

        return view('win_lose', compact('endedAuctions'));
        }
    ...
    public function electronic(Request $request) {
       $paginate = 12;
       $endingSoonest = Auction::where('category','electronic')->where('status', 'active')->orderBy('end_date')->paginate($paginate);
       $endingLatest = Auction::where('category','electronic')->where('status', 'active')->orderByDesc('end_date')->paginate($paginate);
       $new = Auction::where('category','electronic')->where('status', 'active')->orderByDesc('created_at')->paginate($paginate);
       $popular = Auction::withCount('bids')->where('category','electronic')->where('status', 'active')->orderByDesc('bids_count')->paginate($paginate);
       $electronic = Auction::where('category','electronic')->where('status', 'active')->orderBy('created_at')->paginate($paginate);
